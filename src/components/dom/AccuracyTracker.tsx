...
export function useAccuracyTracker(symbol: string = 'BTCUSD') {
    ...
    const verifyPendingPrediction = useCallback(async (predictionId: number) => {
        ...
        try {
            ...
            // Relaxed threshold: 0.2 basis points (0.002%) instead of 1 bp (0.01%)
            // This counts small correct moves as wins, filtering out only flat/spread noise
            const THRESHOLD = 0.2;

            let actualDirection: -1 | 0 | 1;
            if (priceChangePct > THRESHOLD) actualDirection = 1;
            else if (priceChangePct < -THRESHOLD) actualDirection = -1;
