...
export function formatMarketDataForAI(data: MarketData, timeframe: string, market: string = 'IDX'): string {
        ...
        market === 'US' ? 2 : // US Stocks usually 2 decimals
            data.symbol.includes('XAU') || data.symbol.includes('US') ? 2 :
                data.symbol.includes('BTC') ? 2 :
                    0; // Default IDX usually 0 (Rp)

    const candleText = recentCandles.map((c, i) => {
    ...
    return `
=== MARKET DATA ===
MARKET: ${market} (${market === 'US' ? 'US Stock Market' : 'Indonesia Stock Exchange'})
